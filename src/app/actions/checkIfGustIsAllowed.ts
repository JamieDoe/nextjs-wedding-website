...
export default async function checkIfGustIsAllowed(
  ...
  try {
    ...
    const response = (await database.listDocuments(
      ...
      [Query.equal('first_name', first_name), Query.equal('last_name', last_name)]
    )) as { documents: Guest[] }

    if (response.documents.length === 0) {
      return {
        success: false,
        message: "We couldn't find your name in our guest list. Please contact us for further assistance."
      }
    }
